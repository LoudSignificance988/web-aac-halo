...
const SYNC_KEYS=['profiles','currentProfile','customWords','removedWords','wordPack','engine','sysVoice','accent','accentDark','bg','setup','portalHash'];
const DEFAULTS={
  profiles:{},currentProfile:null,engine:'google',sysVoice:null,
...
async function loadFromCloud(){
  ...
  try{
    ...
    if(snap.exists){
      ...
      const keys=['profiles','currentProfile','customWords','removedWords','wordPack','engine','sysVoice','accent','accentDark','bg','setup','portalHash'];
      keys.forEach(k=>{
        if(remote[k]!==undefined&&remote[k]!==null){
          if(remote[k]&&typeof remote[k]==='object'&&!Array.isArray(remote[k])){
...
let curCat=null;
let curEngine=data.engine||'google';
...
const ENG_NAMES={google:'Google',system:'System',responsive:'ResponsiveVoice',puter:'Puter AI'};
...
async function playChunks(chunks,urlFn,label){
  const rate=parseFloat(rateSlider.value);
  const audios=chunks.map(c=>{const a=new Audio(urlFn(c));a.preload='auto';a.load();return a;});
  for(let i=0;i<audios.length;i++){
    if(i>0&&!activeAudio)break;
    setStatus((chunks.length>1?'▶ '+(i+1)+'/'+chunks.length+' · ':'▶ ')+label,true);
    await new Promise(res=>{
      activeAudio=audios[i];
      activeAudio.playbackRate=Math.min(Math.max(rate,.5),4);
      activeAudio.onended=res;activeAudio.onerror=res;
      activeAudio.play().catch(res);
    });
  }
  if(activeAudio)setStatus('Done ✓');
}

async function speakGoogle(text){
  await playChunks(chunkText(text),c=>'https://translate.googleapis.com/translate_tts?ie=UTF-8&tl=en&client=gtx&q='+encodeURIComponent(c),'Google');
}
...
function speakResponsive(text){
  if(typeof responsiveVoice==='undefined'){setStatus('ResponsiveVoice not loaded');return;}
  setStatus('▶ ResponsiveVoice',true);
  responsiveVoice.speak(text,'UK English Female',{
    rate:parseFloat(rateSlider.value)-0.5,
    onend:()=>setStatus('Done ✓'),
    onerror:()=>setStatus('RV Error')
  });
}

async function speakPuter(text){
  if(typeof puter==='undefined'){setStatus('Puter not loaded');speakGoogle(text);return;}
  try{
    setStatus('▶ Puter AI',true);
    const audio=await puter.ai.txt2speech(text);
    activeAudio=audio;
    audio.playbackRate=Math.min(Math.max(parseFloat(rateSlider.value),.5),4);
    audio.onended=()=>setStatus('Done ✓');
    audio.play();
  }catch(e){setStatus('Puter error, falling back');speakGoogle(text);}
}

speakBtn.onclick=async()=>{
  const raw=msgBox.value.trim();
  if(!raw){setStatus('Nothing to say');return;}
  const text=stripEmoji(raw)||raw;
  try{
    if(curEngine==='google') await speakGoogle(text);
    else if(curEngine==='system') speakSystem(text);
    else if(curEngine==='responsive') speakResponsive(text);
    else if(curEngine==='puter') await speakPuter(text);
  }catch(e){setStatus('Trying system…');speakSystem(text);}
};
...
function buildGrid(filter=''){
  ...
  Object.entries(catWords)
    ...
    .forEach(([w,e])=>{
      ...
      btn.onclick=()=>{msgBox.value=msgBox.value?(msgBox.value.trimEnd()+' '+w):w;updatePredictions();};
...
function openSettings(){$('settingsOverlay').classList.add('open');buildSysVoiceList();}
function closeSettings(){$('settingsOverlay').classList.remove('open');}

$('engineGrid').querySelectorAll('.engine-card').forEach(card=>{
  card.onclick=()=>{
    curEngine=card.dataset.eng;data.engine=curEngine;save();
    $('engineGrid').querySelectorAll('.engine-card').forEach(c=>c.classList.remove('active'));
    card.classList.add('active');
    enginePill.textContent='🎙️ '+(ENG_NAMES[curEngine]||curEngine);
    $('sysPickerWrap').classList.toggle('show',curEngine==='system');
  };
});
const savedCard=$('engineGrid').querySelector('[data-eng="'+curEngine+'"]');
...
function init(){
  ...
  enginePill.textContent='🎙️ '+(ENG_NAMES[data.engine]||data.engine);
