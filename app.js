...
const SYNC_KEYS=['profiles','currentProfile','customWords','removedWords','wordPack','sysVoice','accent','accentDark','bg','setup','portalHash','ttsEngine','kokoroVoice'];
const DEFAULTS={
  ...
  ttsEngine:'system',kokoroVoice:'af_heart'
};
...
function loadFromCloud(){
  ...
  _fireDb.collection('users').doc(_fireUser.uid).onSnapshot(snap=>{
    try{
      if(snap.exists){
...
// ── Instant neural audio cache (IndexedDB) ──
// Pre-generates & stores one audio clip per word per voice, so taps are instant.
// Falls back to the fast system voice the first time a word hasn't been cached yet.
const CACHE_DB_NAME='haloTTSCache';
...
function cacheKey(voice,word){return voice+'::'+word.toLowerCase();}

let _pregenRunning=false;
async function pregenerateVoice(voice){
  if(!kokoroTTS||_pregenRunning)return;
  _pregenRunning=true;
  const words=allWords();
  const statusEl=$('kokoroStatus');
  let done=0,cached=0;
  for(const w of words){
    const key=cacheKey(voice,w);
    const existing=await cacheGet(key);
    if(!existing){
      try{
        const out=await kokoroTTS.generate(w,{voice,speed:1.0});
        await cacheSet(key,out.toBlob());
        cached++;
      }catch(e){console.warn('pregen failed for',w,e);}
    }
    done++;
    if(statusEl)statusEl.textContent='⏳ Caching words for instant playback… '+done+'/'+words.length;
    await new Promise(r=>setTimeout(r,0)); // yield so taps stay responsive
  }
  if(statusEl)statusEl.textContent='✓ Neural voice ready — '+words.length+' words cached for instant playback.';
  _pregenRunning=false;
}

function playBlob(blob,rate){
  const url=URL.createObjectURL(blob);
  if(_kokoroAudioEl){_kokoroAudioEl.pause();}
  _kokoroAudioEl=new Audio(url);
  _kokoroAudioEl.playbackRate=rate||1;
  _kokoroAudioEl.onplay=()=>setStatus('▶ Neural',true);
  _kokoroAudioEl.onended=()=>{setStatus('Done ✓');URL.revokeObjectURL(url);};
  _kokoroAudioEl.onerror=()=>setStatus('Playback error');
  _kokoroAudioEl.play();
}

// Single word tap: cache-first for instant playback. If not cached yet, speak
// instantly via the system voice (never make them wait) and cache it for next time.
async function speakWord(word){
  const eng=data.ttsEngine||'system';
  if(eng!=='kokoro'){speakSystem(word);return;}
  const voice=data.kokoroVoice||'af_heart';
  const blob=await cacheGet(cacheKey(voice,word));
  if(blob){playBlob(blob,parseFloat(rateSlider.value));return;}
  speakSystem(word);
  if(kokoroTTS){
    kokoroTTS.generate(word,{voice,speed:1.0}).then(out=>cacheSet(cacheKey(voice,word),out.toBlob())).catch(()=>{});
  }
}

let _stopSequence=false;
function playSequence(blobs){
  _stopSequence=false;
  let i=0;
  const rate=parseFloat(rateSlider.value);
  function playNext(){
    if(_stopSequence||i>=blobs.length){setStatus(_stopSequence?'Stopped':'Done ✓');return;}
    const url=URL.createObjectURL(blobs[i]);
    if(_kokoroAudioEl){_kokoroAudioEl.pause();}
    _kokoroAudioEl=new Audio(url);
    _kokoroAudioEl.playbackRate=rate;
    _kokoroAudioEl.onplay=()=>setStatus('▶ Neural',true);
    _kokoroAudioEl.onended=()=>{URL.revokeObjectURL(url);i++;playNext();};
    _kokoroAudioEl.onerror=()=>{i++;playNext();};
    _kokoroAudioEl.play();
  }
  playNext();
}

// Full message speak: if every word is already cached, chain them together
// instantly. Otherwise fall back to live generation for the whole sentence.
async function speak(text){
  const eng=data.ttsEngine||'system';
  if(eng!=='kokoro'){speakSystem(text);return;}
  const voice=data.kokoroVoice||'af_heart';
  const words=text.trim().split(/\s+/);
  const blobs=[];
  for(const w of words){
    const b=await cacheGet(cacheKey(voice,w));
    if(!b){playKokoro(text);return;}
    blobs.push(b);
  }
  playSequence(blobs);
}
...
speakBtn.onclick=()=>{
  ...
  speak(stripEmoji(raw)||raw);
};

stopBtn.onclick=()=>{
  speechSynthesis.cancel();
  _stopSequence=true;
  if(_kokoroAudioEl){_kokoroAudioEl.pause();_kokoroAudioEl.currentTime=0;}
  setStatus('Stopped');
};
...
function buildGrid(filter=''){
  ...
  Object.entries(catWords)
    ...
    .forEach(([w,e])=>{
      ...
      btn.onclick=()=>{msgBox.value=msgBox.value?(msgBox.value.trimEnd()+' '+w):w;updatePredictions();speakWord(w);};
...
function openSettings(){
  ...
  const eng=data.ttsEngine||'system';
  document.querySelectorAll('#engineGrid .engine-card').forEach(c=>c.classList.toggle('active',c.dataset.eng===eng));
  $('sysPickerWrap').classList.toggle('show',eng==='system');
  $('kokoroPickerWrap').classList.toggle('show',eng==='kokoro');
  buildSysVoiceList();
  if(eng==='kokoro')ensureKokoro();
}
function closeSettings(){$('settingsOverlay').classList.remove('open');}

function selectEngine(eng){
  data.ttsEngine=eng;save();
  document.querySelectorAll('#engineGrid .engine-card').forEach(c=>c.classList.toggle('active',c.dataset.eng===eng));
  $('sysPickerWrap').classList.toggle('show',eng==='system');
  $('kokoroPickerWrap').classList.toggle('show',eng==='kokoro');
  if(eng==='kokoro')ensureKokoro();
}
document.querySelectorAll('#engineGrid .engine-card').forEach(card=>{
  card.onclick=()=>selectEngine(card.dataset.eng);
});

// ── Kokoro neural voice engine (open-source, runs 100% client-side via WASM/WebGPU) ──

const KOKORO_VOICES=[
  {id:'af_heart',name:'Heart',lang:'EN-US · Female'},
  {id:'af_bella',name:'Bella',lang:'EN-US · Female'},
  {id:'am_michael',name:'Michael',lang:'EN-US · Male'},
  {id:'am_fenrir',name:'Fenrir',lang:'EN-US · Male'},
  {id:'bf_emma',name:'Emma',lang:'EN-GB · Female'},
  {id:'bm_george',name:'George',lang:'EN-GB · Male'}
];

async function ensureKokoro(){
  if(kokoroTTS){buildKokoroVoiceList();return;}
  if(kokoroLoading)return;
  kokoroLoading=true;
  const statusEl=$('kokoroStatus');
  if(statusEl)statusEl.textContent='⏳ Downloading neural voice model (~80MB, first time only)…';
  try{
    const mod=await import('https://cdn.jsdelivr.net/npm/kokoro-js/+esm');
    kokoroTTS=await mod.KokoroTTS.from_pretrained('onnx-community/Kokoro-82M-v1.0-ONNX',{dtype:'q8',device:'wasm'});
    buildKokoroVoiceList();
    pregenerateVoice(data.kokoroVoice||'af_heart');
  }catch(e){
    console.warn('Kokoro load failed:',e);
    if(statusEl)statusEl.textContent='⚠️ Could not load the neural voice (needs internet for the first download). Switched back to System.';
    data.ttsEngine='system';save();
    document.querySelectorAll('#engineGrid .engine-card').forEach(c=>c.classList.toggle('active',c.dataset.eng==='system'));
    $('sysPickerWrap').classList.add('show');
    $('kokoroPickerWrap').classList.remove('show');
  }finally{
    kokoroLoading=false;
  }
}

function buildKokoroVoiceList(){
  const vl=$('kokoroVoiceList');if(!vl)return;
  vl.innerHTML='';
  KOKORO_VOICES.forEach(v=>{
    const row=document.createElement('div');row.className='v-row'+(v.id===(data.kokoroVoice||'af_heart')?' sel':'');
    row.innerHTML='<span class="v-name">'+v.name+'</span><span class="v-lang">'+v.lang+'</span><button class="v-play">▶</button>';
    row.querySelector('.v-play').onclick=e=>{e.stopPropagation();playKokoro('Hello, I am '+v.name+'.',v.id);};
    row.onclick=()=>{data.kokoroVoice=v.id;save();vl.querySelectorAll('.v-row').forEach(r=>r.classList.remove('sel'));row.classList.add('sel');pregenerateVoice(v.id);};
    vl.appendChild(row);
  });
}

async function playKokoro(text,voiceOverride){
  if(!kokoroTTS){
    setStatus('Loading neural voice…');
    await ensureKokoro();
    if(!kokoroTTS)return;
  }
  try{
    setStatus('⏳ Generating…');
    const voice=voiceOverride||data.kokoroVoice||'af_heart';
    const speed=parseFloat(rateSlider.value);
    const out=await kokoroTTS.generate(text,{voice,speed});
    const blob=out.toBlob();
    const url=URL.createObjectURL(blob);
    if(_kokoroAudioEl){_kokoroAudioEl.pause();}
    _kokoroAudioEl=new Audio(url);
    _kokoroAudioEl.onplay=()=>setStatus('▶ '+voice,true);
    _kokoroAudioEl.onended=()=>setStatus('Done ✓');
    _kokoroAudioEl.onerror=()=>setStatus('Playback error');
    await _kokoroAudioEl.play();
  }catch(e){
    console.warn('Kokoro speak failed:',e);
    setStatus('Neural voice error — using system');
    speakSystem(text);
  }
}
...
function kickPregen(){
  if((data.ttsEngine||'system')==='kokoro'&&kokoroTTS)pregenerateVoice(data.kokoroVoice||'af_heart');
}
...
function wFinish(){
  ...
  buildCats();buildGrid();refreshPD();kickPregen();
}
...
function openPortal(){
  ...
  buildWordTable();
  updatePackLabel();
  refreshPortalProfileSelect();
  $('portalOverlay').classList.add('open');
}
function closePortal(){$('portalOverlay').classList.remove('open');}

function refreshPortalProfileSelect(){
  const sel=$('portalProfileSelect');if(!sel)return;
  sel.innerHTML='';
  Object.keys(data.profiles).forEach(n=>{const o=document.createElement('option');o.value=n;o.textContent=n;sel.appendChild(o);});
  if(data.currentProfile)sel.value=data.currentProfile;
}
function portalDelProfile(){
  const sel=$('portalProfileSelect');
  const target=sel?sel.value:data.currentProfile;
  if(!target||!confirm('Delete profile "'+target+'"? This cannot be undone.'))return;
  delete data.profiles[target];
  if(data.currentProfile===target)data.currentProfile=Object.keys(data.profiles)[0]||null;
  save();
  refreshPS();refreshPortalProfileSelect();buildCats();buildGrid();refreshPD();
}
...
function switchTab(el,panelId){
  ...
  if(panelId==='tabSettings'){updatePackLabel();refreshPortalProfileSelect();}
}
